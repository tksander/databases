...
var app = {
  ...
  init: function() {
    // Get username
    // window.location.search.substr(10)
  ...
  fetch: function(animate) {
    $.ajax({
      ...
      success: function(data) {
        ...
        if (mostRecentMessage.objectId !== app.lastMessageId || app.room !== displayedRoom) {
  ...
  addMessage: function(data) {
    ...
    if (data.room === app.room) {
      ...
      $username.text(data.user+': ').attr('data-username', data.user).attr('data-room',data.room).appendTo($chat);
      ...
      var $message = $('<br><span/>');
      $message.text(data.messages).appendTo($chat);
